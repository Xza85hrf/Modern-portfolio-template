...
export function registerRoutes(app: Express) {
  ...
  app.post("/api/projects/sync-github-all", authMiddleware, async (req, res) => {
    try {
      const username = req.body.username || process.env.GITHUB_USERNAME;
      if (!username) {
        return res.status(400).json({
          message: "GitHub username required. Set GITHUB_USERNAME environment variable or provide username in request body.",
        });
      }
