...
import { useHero } from "@/lib/config-context";
...
export function HeroContent({
  ...
  const displaySubtitle = subtitle ?? hero.badge;
    ...
    >
      ...
      <motion.div variants={staggerItemVariants} className="mb-6">
        <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-sm font-medium">
          ...
          <span className="gradient-text">{displaySubtitle}</span>
        </span>
      </motion.div>
      ...
      >
        {headline.map((word, index) => (
          <span
            key={index}
            className={`block ${index === 1 ? "gradient-text" : "text-foreground"}`}
          >
            {word}
          </span>
        ))}
